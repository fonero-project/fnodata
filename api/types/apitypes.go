...
// much of the time, fnodata will be using the types in fnojson, but others are
// defined here
...
// of this type are NOT compatible with fnod's txscript.ScriptClass values! Use
...
// class. If fnod's txscript package changes its strings, this function may be
// unable to identify the types from fnod.
...
// TxOut defines a fonero transaction output.
type TxOut struct {
	Value               float64      `json:"value"`
	Version             uint16       `json:"version"`
	ScriptPubKeyDecoded ScriptPubKey `json:"scriptPubKey"`
...
// TxIn defines a fonero transaction input.
type TxIn struct {
	// Non-witness
...
type AddressTxRaw struct {
	...
	Version       int32                `json:"version"`
	Locktime      uint32               `json:"locktime"`
	Vin           []fnojson.VinPrevOut `json:"vin"`
	Vout          []Vout               `json:"vout"`
	Confirmations int64                `json:"confirmations"`
	BlockHash     string               `json:"blockhash"`
...
type AddressTotals struct {
	...
	CoinsSpent   float64 `json:"fno_spent"`
	CoinsUnspent float64 `json:"fno_unspent"`
}

// BlockDataWithTxType adds an array of TxRawWithTxType to
// fnojson.GetBlockVerboseResult to include the stake transaction type
type BlockDataWithTxType struct {
	*fnojson.GetBlockVerboseResult
	Votes   []TxRawWithVoteInfo
	Tickets []fnojson.TxRawResult
	Revs    []fnojson.TxRawResult
}

// TxRawWithVoteInfo adds the vote info to fnojson.TxRawResult
type TxRawWithVoteInfo struct {
	fnojson.TxRawResult
	VoteInfo VoteInfo
}

// TxRawWithTxType adds the stake transaction type to fnojson.TxRawResult
...
type APIStatus struct {
	...
	FnodataVersion  string `json:"fnodata_version"`
	NetworkName     string `json:"network_name"`
}

// NewStatus is the constructor for a new Status.
func NewStatus(nodeHeight uint32, conns int64, apiVersion int, fnodataVersion, netName string) *Status {
	return &Status{
		height:          nodeHeight,
		nodeConnections: conns,
		api: APIStatus{
			APIVersion:     apiVersion,
			FnodataVersion: fnodataVersion,
			NetworkName:    netName,
		},
	}
}
...
func (s *Status) API() APIStatus {
	...
	return APIStatus{
		...
		FnodataVersion:  s.api.FnodataVersion,
...
type BlockExplorerExtraInfo struct {
	...
	NextBlockSubsidy *fnojson.GetBlockSubsidyResult `json:"next_block_subsidy"`
}
...
type StakeDiff struct {
	...
	Estimates        fnojson.EstimateStakeDiffResult `json:"estimates"`
...
type StakeInfoExtended struct {
	Hash             string               `json:"hash"`
	Feeinfo          fnojson.FeeInfoBlock `json:"feeinfo"`
