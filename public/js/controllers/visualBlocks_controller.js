...
function makeMempoolBlock (block) {
  ...
  return makeNode(`<div class="block visible">
                <div class="block-info">
                    ...
                    <div class="mono" style="line-height: 1;">${Math.floor(block.Total)} FNO</div>
                    <span class="timespan">
...
function makeBlockSummary (blockHeight, totalSent, time) {
  return `<div class="block-info">
                ...
                <div class="mono" style="line-height: 1;">${Math.floor(totalSent)} FNO</div>
                <span class="timespan">
...
function makeRewardsElement (subsidy, fee, voteCount, rewardTxId) {
  ...
  // const totalFNO = Math.round(pow + fund + fee);
  const totalFNO = 1
  return `<div class="block-rewards" style="flex-grow: ${totalFNO}">
...
function makeVoteElements (votes) {
  ...
  // totalFNO = Math.round(totalFNO);
  totalFNO = 1
  return `<div class="block-votes" style="flex-grow: ${totalFNO}">
                ${voteElements.join('\n')}
            </div>`
}
...
function makeTicketAndRevocationElements (tickets, revocations, blockHref) {
  ...
  // totalFNO = Math.round(totalFNO);
  totalFNO = 1
  return `<div class="block-tickets" style="flex-grow: ${totalFNO}">
...
function makeTransactionElements (transactions, blockHref) {
  ...
  // totalFNO = Math.round(totalFNO);
  totalFNO = 1
  return `<div class="block-transactions" style="flex-grow: ${totalFNO}">
    ...
    this.tooltipTargets.forEach((tooltipElement) => {
      try {
        ...
        if (data.object === 'Vote') {
          ...
          newContent = `<b>${data.object}</b><br>${data.total} FNO`
        }
