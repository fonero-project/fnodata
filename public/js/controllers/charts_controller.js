...
function legendFormatter (data) {
  ...
  if (data.x == null) {
    ...
    let yVals = data.series.reduce((nodes, series) => {
      ...
      let yVal = series.label.toLowerCase().includes('coin supply') ? intComma(series.y) + ' FNO' : series.yHTML
...
function circulationFunc (gData, blocks) {
  ...
  var addDough = (newHeight) => {
    while (h < newHeight) {
      h++
      circ += blockReward(h) * atomsToFNO
    }
  }
  var data = map(gData.x, (t, i) => {
    addDough(blocks ? i : gData.z[i])
    return [new Date(t * 1000), gData.y[i] * atomsToFNO, circ]
  })
  ...
  plotGraph (chartName, data) {
    ...
    switch (chartName) {
      case 'ticket-price': // price graph
        d = zipYvDate(data, atomsToFNO)
        gOptions.stepPlot = true
        assign(gOptions, mapDygraphOptions(d, ['Date', 'Ticket Price'], true, 'Price (FNO)', 'Date', undefined, false, false))
        break
      ...
      case 'coin-supply': // supply graph
        ...
        assign(gOptions, mapDygraphOptions(d, ['Date', 'Coin Supply', 'Predicted Coin Supply'], true, 'Coin Supply (FNO)', 'Date', undefined, true, false))
        break

      case 'fees': // block fee graph
        d = zipYvDate(data, atomsToFNO)
        assign(gOptions, mapDygraphOptions(d, ['Block Height', 'Total Fee'], false, 'Total Fee (FNO)', 'Block Height',
          undefined, true, false))
        break
