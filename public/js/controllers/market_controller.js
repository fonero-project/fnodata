...
const exchangeLinks = {
  binance: 'https://www.binance.com/en/trade/FNO_BTC',
  bittrex: 'https://bittrex.com/Market/Index?MarketName=BTC-FNO',
  poloniex: 'https://poloniex.com/exchange#btc_fno',
  dragonex: 'https://dragonex.io/en-us/trade/index/fno_btc',
  huobi: 'https://www.hbg.com/en-us/exchange/?s=fno_btc'
}
...
export default class extends Controller {
  ...
  processVolume (response) {
    ...
    return {
      ...
      labels: ['time', 'volume'],
      xlabel: 'Time',
      ylabel: `Volume (FNO / ${prettyDurations[settings.bin]})`,
      colors: [chartStroke],
      plotter: Dygraph.Plotters.linePlotter,
      axes: {
        x: {
  ...
  _processXcUpdate (update) {
    ...
    if (update.fiat) { // btc-fiat exchange update
      ...
      if (xc.change >= 0) {
        row.arrow.className = 'fnoicon-arrow-up text-green'
      } else {
        row.arrow.className = 'fnoicon-arrow-down text-danger'
      }
    }
