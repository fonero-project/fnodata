...
function customizedFormatter (data) {
  ...
  data.series.map((series) => {
    ...
    var l = `<span style="color: ` + series.color + ';"> ' + series.labelHTML
    html = `<span style="color:#2d2d2d;">` + html + `</span>`
    html += '<br>' + series.dashHTML + l + ': ' + (isNaN(series.y) ? '' : series.y + ' FNO') + '</span> '
  })
  return html
}
...
export default class extends Controller {
  ...
  async connect () {
    ...
    // Parse stimulus data
    var cdata = ctrl.data
    ctrl.fnoAddress = cdata.get('fnoaddress')
  ...
  async showQRCode () {
    ...
    if (this.qrCode) {
      ...
      let qrCodeImg = await QRCode.toDataURL(this.fnoAddress,
        {
  ...
  makeTableUrl (txType, count, offset) {
    return '/addresstable/' + this.fnoAddress + '?txntype=' +
  ...
  async fetchGraphData (chart, bin) {
    ...
    let url = '/api/address/' + ctrl.fnoAddress + '/' + chartKey + '/' + bin
