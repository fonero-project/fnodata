...
// Copyright (c) 2018, The fnodata developers
...
type StakeDatabase struct {
	...
	BestNode        *stake.Node
	blkMtx          sync.RWMutex
	blockCache      map[int64]*fnoutil.Block
...
func LoadAndRecover(client *rpcclient.Client, params *chaincfg.Params,
	...
	sDB := &StakeDatabase{
		params:          params,
		NodeClient:      client,
		blockCache:      make(map[int64]*fnoutil.Block),
...
func NewStakeDatabase(client *rpcclient.Client, params *chaincfg.Params,
	...
	if heightStakeDB != heightTicketPool {
		if heightStakeDB-heightTicketPool > 16 {
			...
			_ = sDB.Close()
			_ = poolDB.Close()
			return nil, heightTicketPool, fmt.Errorf("Remove %s and start fnodata again.",
...
func (db *StakeDatabase) PopulateLiveTicketCache() error {
	var err error
	// Live tickets from fnodata's stake Node's perspective
...
func (db *StakeDatabase) BlockCached(ind int64) (*fnoutil.Block, bool) {
	db.blkMtx.RLock()
...
func (db *StakeDatabase) block(ind int64) (*fnoutil.Block, bool) {
...
func (db *StakeDatabase) ConnectBlockHash(hash *chainhash.Hash) (*fnoutil.Block, error) {
...
func (db *StakeDatabase) ConnectBlock(block *fnoutil.Block) error {
	height := block.Height()
...
func (db *StakeDatabase) connectBlock(block *fnoutil.Block, spent []chainhash.Hash,
...
func (db *StakeDatabase) Open(dbName string) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "resource temporarily unavailable") ||
			...
			return fmt.Errorf("Stake DB already opened. fnodata running?")
		}
...
func (db *StakeDatabase) makePoolInfo(poolValue, poolSize int64,
	...
	poolCoin := fnoutil.Amount(poolValue).ToCoin()
	valAvg := 0.0
	if poolSize > 0 {
...
// DBTipBlock gets the fnoutil.Block for the current best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBTipBlock() (*fnoutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
// DBPrevBlock gets the fnoutil.Block for the previous best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBPrevBlock() (*fnoutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
func (db *StakeDatabase) getBlock(hash *chainhash.Hash) (*fnoutil.Block, error) {
	msgBlock, err := db.NodeClient.GetBlock(hash)
	if err == nil {
		return fnoutil.NewBlock(msgBlock), nil
	}
	return nil, err
}
