...
// by fnodata or rebuilddb2. The patch versions may also be different. They
...
// CreateTablesLegacy creates all tables required by fnodata if they do not
// already exist.
...
// style is run after updating fnodata past version 3.0.0.
...
// lnfeatures agenda ids exist since appropriate voting mechanisms on the fnod
...
func (pgb *ChainDB) UpgradeTables(fnodClient BlockGetter, version, needVersion TableVersion) (bool, error) {
	// If the previous DB is between the 3.1/3.2 and 4.0 releases (fnopg table
		...
		if dataType == "timestamp without time zone" {
			...
			if defaultTZ != "UTC" {
				...
				return false, fmt.Errorf(
					"your fnopg schema (v%v) has time columns without time zone, "+
	...
	case version.major == 3 && version.minor == 1 && version.patch == 0:
		...
		isSuccess, er := pgb.initiatePgUpgrade(fnodClient, theseUpgrades)
		if !isSuccess {
			return isSuccess, er
		}
	...
	case version.major == 3 && version.minor == 5 && version.patch == 4:
		toVersion = TableVersion{3, 5, 5} // fnodata 3.1 release
	...
	case version.major == 3 && version.minor == 9 && version.patch == 0:
		...
		rawChainInfo, err := fnodClient.GetBlockChainInfo()
		if err != nil {
			return false, err
		}
...
func verifyChainWork(client BlockGetter, db *sql.DB) (int64, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			// If it's an orphaned block, it may not be in fnod database. OK to skip.
