...
func (p *ChainMonitor) switchToSideChain(reorgData *txhelpers.ReorgData) (int32, *chainhash.Hash, error) {
	...
	defer func(start time.Time) {
		log.Infof("fnopg: switchToSideChain completed in %v", time.Since(start))
	}(time.Now())
	...
	for i := range newChain {
		...
		if !found || msgBlock.BlockHash() != newChain[i] {
			log.Debugf("block %v not found in stakedb cache, fetching from fnod", newChain[i])
			// Request MsgBlock from fnod
