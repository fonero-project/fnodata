...
func (db *wiredDB) GetAllTxOut(txid string) []*apitypes.TxOut {
	...
	for i := range allTxOut {
		...
		txOut := &apitypes.TxOut{
			Value:     fnoutil.Amount(allTxOut0[i].Value).ToCoin(),
...
func (db *wiredDB) getRawTransaction(txid string) (*apitypes.Tx, string) {
	...
	tx.Vin = make([]fnojson.Vin, len(txraw.Vin))
...
func (db *wiredDB) GetStakeVersions(txHash string, count int32) (*fnojson.GetStakeVersionsResult, error) {
...
func makeExplorerBlockBasic(data *fnojson.GetBlockVerboseResult, params *chaincfg.Params) *explorer.BlockBasic {
...
func makeExplorerTxBasic(data fnojson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *explorer.TxBasic {
...
func makeExplorerAddressTx(data *fnojson.SearchRawTransactionsResult, address string) *explorer.AddressTx {
...
func (db *wiredDB) GetExplorerBlock(hash string) *explorer.BlockInfo {
	...
	getTotalFee := func(txs []*explorer.TrimmedTxInfo) (total fnoutil.Amount) {
		for _, tx := range txs {
			total += tx.Fee
		}
		return
	}
	getTotalSent := func(txs []*explorer.TrimmedTxInfo) (total fnoutil.Amount) {
		for _, tx := range txs {
...
func trimmedTxInfoFromMsgTx(txraw fnojson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *explorer.TrimmedTxInfo {
...
func (db *wiredDB) GetExplorerTx(txid string) *explorer.TxInfo {
	...
	for i, vin := range txraw.Vin {
		var addresses []string
		// ValueIn is a temporary fix until amountIn is correct from fnod call
		var ValueIn fnoutil.Amount
		if !(vin.IsCoinBase() || (vin.IsStakeBase() && i == 0)) {
			var addrs []string
...
func (db *wiredDB) UnconfirmedTxnsForAddress(address string) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := db.client.GetRawMempoolVerbose(fnojson.GRMAll)
	if err != nil {
