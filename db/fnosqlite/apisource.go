...
// parameters, and a status update channel. It calls fnosqlite.NewDB to create a
...
func (db *WiredDB) ReportHeights() error {
	...
	if stakeInfoHeight != summaryHeight {
		err = fmt.Errorf("SQLite database (fnodata.sqlt.db) is corrupted")
	}
...
func (db *WiredDB) BlockchainInfo() (*fnojson.GetBlockChainInfoResult, error) {
...
func (db *WiredDB) GetHeader(idx int) *fnojson.GetBlockHeaderVerboseResult {
	return rpcutils.GetBlockHeaderVerbose(db.client, int64(idx))
}

func (db *WiredDB) GetBlockVerbose(idx int, verboseTx bool) *fnojson.GetBlockVerboseResult {
	return rpcutils.GetBlockVerbose(db.client, int64(idx), verboseTx)
}

func (db *WiredDB) GetBlockVerboseByHash(hash string, verboseTx bool) *fnojson.GetBlockVerboseResult {
...
func (db *WiredDB) BlockSubsidy(height int64, voters uint16) *fnojson.GetBlockSubsidyResult {
...
func (db *WiredDB) GetAllTxIn(txid *chainhash.Hash) []*apitypes.TxIn {
	...
	for i := range allTxIn {
		txIn := &apitypes.TxIn{
			...
			ValueIn:         fnoutil.Amount(allTxIn0[i].ValueIn).ToCoin(),
...
func (db *WiredDB) GetAllTxOut(txid *chainhash.Hash) []*apitypes.TxOut {
	...
	for i := range txouts {
		// fnojson.Vout and apitypes.TxOut are the same except for N.
		spk := &tx.Vout[i].ScriptPubKey
		// If the script type is not recognized by apitypes, the ScriptClass
		// types may need to be updated to match fnod.
		if spk.Type != "invalid" && !apitypes.IsValidScriptClass(spk.Type) {
			log.Warnf(`The ScriptPubKey's type "%s" is not known to fnodata! ` +
				`Update apitypes or debug fnod.`)
		}
...
func (db *WiredDB) DecodeRawTransaction(txhex string) (*fnojson.TxRawResult, error) {
...
// GetVoteVersionInfo requests stake version info from the fnod RPC server
func (db *WiredDB) GetVoteVersionInfo(ver uint32) (*fnojson.GetVoteInfoResult, error) {
...
func (db *WiredDB) GetStakeVersions(blockHash string, count int32) (*fnojson.GetStakeVersionsResult, error) {
...
func (db *WiredDB) GetStakeVersionsLatest() (*fnojson.StakeVersions, error) {
...
// on the stake version with which fnodata is compiled with (chaincfg.Params),
...
func (db *WiredDB) GetFeeInfo(idx int) *fnojson.FeeInfoBlock {
...
func (db *WiredDB) GetSummaryByHash(hash string, withTxTotals bool) *apitypes.BlockDataBasic {
	...
	if withTxTotals {
		...
		var totalFees, totalOut fnoutil.Amount
...
func (db *WiredDB) GetAddressTransactionsRawWithSkip(addr string, count int, skip int) []*apitypes.AddressTxRaw {
	...
	for i := range txs {
		...
		tx.Vin = make([]fnojson.VinPrevOut, len(txs[i].Vin))
...
func sumOutsTxRawResult(txs []fnojson.TxRawResult) (sum float64) {
	for _, tx := range txs {
...
func makeExplorerBlockBasic(data *fnojson.GetBlockVerboseResult, params *chaincfg.Params) *exptypes.BlockBasic {
...
func makeExplorerTxBasic(data fnojson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TxBasic {
...
func makeExplorerAddressTx(data *fnojson.SearchRawTransactionsResult, address string) *dbtypes.AddressTx {
...
func (db *WiredDB) GetExplorerBlock(hash string) *exptypes.BlockInfo {
	...
	for _, tx := range data.RawSTx {
		...
		case stake.TxTypeSSGen:
			...
			// well.
			if stx.Fee > 0 {
				log.Tracef("Vote with fee! %v, %v FNO", stx.Fee, stx.Fees)
			}
	...
	getTotalFee := func(txs []*exptypes.TrimmedTxInfo) (total fnoutil.Amount) {
		for _, tx := range txs {
	...
	getTotalSent := func(txs []*exptypes.TrimmedTxInfo) (total fnoutil.Amount) {
		for _, tx := range txs {
			amt, err := fnoutil.NewAmount(tx.Total)
			if err != nil {
				continue
			}
			total += amt
...
func trimmedTxInfoFromMsgTx(txraw fnojson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TrimmedTxInfo {
...
func (db *WiredDB) GetExplorerTx(txid string) *exptypes.TxInfo {
	...
	for i, vin := range txraw.Vin {
		...
		valueIn, _ := fnoutil.NewAmount(vin.AmountIn)
...
func (db *WiredDB) GetExplorerAddress(address string, count, offset int64) (*dbtypes.AddressInfo, txhelpers.AddressType, txhelpers.AddressError) {
	...
	var totalreceived, totalsent fnoutil.Amount
		...
		for _, u := range tx.Vin {
			if u.PrevOut != nil && len(u.PrevOut.Addresses) != 0 {
				if address == u.PrevOut.Addresses[0] {
					t, _ := fnoutil.NewAmount(*u.AmountIn)
					if t > 0 {
						totalsent += t
					}
					numSpending++
...
func (db *WiredDB) GetMempool() []exptypes.MempoolTx {
	mempooltxs, err := db.client.GetRawMempoolVerbose(fnojson.GRMAll)
	if err != nil {
