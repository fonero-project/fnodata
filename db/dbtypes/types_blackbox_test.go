...
func openDB() (func() error, error) {
	...
	db, err = fnopg.Connect(dbi.Host, dbi.Port, dbi.User, dbi.Pass, dbi.DBName)
	if err != nil {
		return nil, err
	}
	if err = fnopg.DropTestingTable(db); err != nil {
		return nil, err
	}
	if err = fnopg.CreateTable(db, "testing"); err != nil {
		return nil, err
	}
...
func TestTimeRoundTripCorrectTimeDef(t *testing.T) {
	...
	if err := fnopg.ClearTestingTable(db); err != nil {
