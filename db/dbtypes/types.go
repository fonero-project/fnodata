...
import (
	...
	"github.com/fonero-project/fnodata/db/dbtypes/internal"
...
const (
	...
	// be effected. https://docs.fonero.org/glossary/#rule-change-interval-rci.
...
// Tx models a Fonero transaction. It is stored in a Block.
type Tx struct {
	//blockDbID  int64
...
type AddressInfo struct {
	...
	AmountReceived  fnoutil.Amount
	AmountSent      fnoutil.Amount
	AmountUnspent   fnoutil.Amount
...
func ReduceAddressHistory(addrHist []*AddressRow) (*AddressInfo, float64, float64) {
	...
	for _, addrOut := range addrHist {
		...
		coin := fnoutil.Amount(addrOut.Value).ToCoin()
	...
	ai := &AddressInfo{
		...
		AmountReceived:  fnoutil.Amount(received),
		AmountSent:      fnoutil.Amount(sent),
		AmountUnspent:   fnoutil.Amount(received - sent),
	}
