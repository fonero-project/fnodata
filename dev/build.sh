#!/bin/bash
# Usage: build.sh [fnodata_root] [destination_folder]
#
...
#   root folder. If not running from a git repository, the fnodata_root folder
#   must be specified.
#
#   Specify destination_folder to install the fnodata executable and the static
...
if [[ -z "$ROOT" ]]; then
    echo "Not in git repository. You must specify the fnodata root folder as the first argument!"
    exit 1
fi
...
echo "Building the fnodata binary..."
GO111MODULE=on go build -v -o fnodata
...
if [[ -n "$DEST" ]]; then
    sudo install -m 754 -o fnodata -g fonero ./fnodata ${DEST}/
