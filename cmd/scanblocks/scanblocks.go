...
var user = flag.String("user", "fnod", "node RPC username")
var pass = flag.String("pass", "bananas", "node RPC password")
var cert = flag.String("cert", "fnod.cert", "node RPC TLS certificate (when notls=false)")
...
func mainCore() int {
	...
	blocks := make(map[int64]*fnoutil.Block)
	...
	for i := int64(0); i < height+1; i++ {
		...
		blocks[i] = fnoutil.NewBlock(msgBlock)
	...
	for i := range blockSummaries {
		blockSummaries[i].PoolInfo.Value = fnoutil.Amount(poolValues[i]).ToCoin()
