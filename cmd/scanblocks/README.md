...
block syncd by fnod, to extract basic data from each block.

## Usage

You just need to specify the user, password, host:port, and (optionally) the RPC
TLS certificate if using a TLS connection to fnod.

```sh
./scanblocks -user fnoduser -pass fnodpass
```

The argument `-notls` is true by default, but if you set it to false, it is
required to specify the RPC cert for fnod with `-cert`.  I strongly suggest to
run fnod with `--notls` when using `scanblocks` for performance reasons.
