...
var (
	...
	BinanceURLs = URLs{
		Price: "https://api.binance.com/api/v1/ticker/24hr?symbol=FNOBTC",
		// Binance returns a maximum of 1000 depth chart points. This seems like it
		// is the entire order book at least sometimes.
		Depth: "https://api.binance.com/api/v1/depth?symbol=FNOBTC&limit=1000",
		Candlesticks: map[candlestickKey]string{
			hourKey:  "https://api.binance.com/api/v1/klines?symbol=FNOBTC&interval=1h",
			dayKey:   "https://api.binance.com/api/v1/klines?symbol=FNOBTC&interval=1d",
			monthKey: "https://api.binance.com/api/v1/klines?symbol=FNOBTC&interval=1M",
		},
	}
	BittrexURLs = URLs{
		Price: "https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-fno",
		// Bittrex gives no documentation on the amount of data that will be
		// returned, and has no parameters to get more data.
		Depth: "https://bittrex.com/api/v1.1/public/getorderbook?market=btc-fno&type=both",
		...
		Candlesticks: map[candlestickKey]string{
			hourKey: "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-FNO&tickInterval=hour",
			dayKey:  "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-FNO&tickInterval=day",
		},
	...
	HuobiURLs = URLs{
		Price: "https://api.huobi.pro/market/detail/merged?symbol=fnobtc",
		// Huobi's only depth parameter defines bin size, 'step0' seems to mean bin
		// width of zero.
		Depth: "https://api.huobi.pro/market/depth?symbol=fnobtc&type=step0",
		Candlesticks: map[candlestickKey]string{
			hourKey:  "https://api.huobi.pro/market/history/kline?symbol=fnobtc&period=60min&size=2000",
			dayKey:   "https://api.huobi.pro/market/history/kline?symbol=fnobtc&period=1day&size=2000",
			monthKey: "https://api.huobi.pro/market/history/kline?symbol=fnobtc&period=1mon&size=2000",
		},
	}
	PoloniexURLs = URLs{
		Price: "https://poloniex.com/public?command=returnTicker",
		// Maximum value of 100 for depth parameter.
		Depth: "https://poloniex.com/public?command=returnOrderBook&currencyPair=BTC_FNO&depth=100",
		Candlesticks: map[candlestickKey]string{
			halfHourKey: "https://poloniex.com/public?command=returnChartData&currencyPair=BTC_FNO&period=1800&start=0&resolution=auto",
			dayKey:      "https://poloniex.com/public?command=returnChartData&currencyPair=BTC_FNO&period=86400&start=0&resolution=auto",
		},
...
// FnoExchanges maps tokens to constructors for FNO-BTC exchanges.
var FnoExchanges = map[string]func(*http.Client, *BotChannels) (Exchange, error){
	Binance:  NewBinance,
	Bittrex:  NewBittrex,
...
// opposed to a Fonero-to-Bitcoin Exchange.
func IsBtcIndex(token string) bool {
	_, ok := BtcIndices[token]
	return ok
}

// IsFnoExchange checks whether the given token is a known Fonero-BTC exchange.
func IsFnoExchange(token string) bool {
	_, ok := FnoExchanges[token]
	return ok
}

// Tokens is a new slice of available exchange tokens.
func Tokens() []string {
	tokens := make([]string, 0, len(BtcIndices)+len(FnoExchanges))
	var token string
	for token = range BtcIndices {
		tokens = append(tokens, token)
	}
	for token = range FnoExchanges {
		tokens = append(tokens, token)
	}
	return tokens
}
...
// guaranteed is a price. For Fonero exchanges, the volumes will also be
// populated.
...
func exchangeStateFromProto(proto *fnorates.ExchangeRateUpdate) *ExchangeState {
	state := &ExchangeState{
...
func (binance *BinanceExchange) Refresh() {
	...
	fnoVolume, err := strconv.ParseFloat(priceResponse.Volume, 64)
	if err != nil {
...
type HuobiCandlestickPt struct {
	...
	Vol    float64 `json:"vol"`    // Volume FNO
	Count  int64   `json:"count"`
}
...
// sporadically, but it is used. For the BTC_FNO endpoint, almost all updates
