...
const (
	...
	defaultFNORatesPort = "7778"

	aggregatedOrderbookKey = "aggregated"
	orderbookKey           = "depth"
)

var grpcClient fnorates.FNORatesClient
...
type ExchangeBot struct {
	...
	// exchanges or no up-to-date Fonero exchanges. IsFailed is a getter for failed.
	failed bool
}

// ExchangeBotState is the current known state of all exchanges, in a certain
// base currency, and a volume-averaged price and total volume in FNO.
type ExchangeBotState struct {
	BtcIndex    string                    `json:"btc_index"`
	BtcPrice    float64                   `json:"btc_fiat_price"`
	Price       float64                   `json:"price"`
	Volume      float64                   `json:"volume"`
	FnoBtc      map[string]*ExchangeState `json:"fno_btc_exchanges"`
...
func (state ExchangeBotState) copy() *ExchangeBotState {
	state.FnoBtc = copyStates(state.FnoBtc)
...
func (state *ExchangeBotState) VolumeOrderedExchanges() []*tokenedExchange {
	xcList := make([]*tokenedExchange, 0, len(state.FnoBtc))
	for token, state := range state.FnoBtc {
...
func NewExchangeBot(config *ExchangeBotConfig) (*ExchangeBot, error) {
	...
	bot := &ExchangeBot{
		FnoBtcExchanges: make(map[string]Exchange),
		...
		currentState: ExchangeBotState{
			...
			Price:       0,
			Volume:      0,
			FnoBtc:      make(map[string]*ExchangeState),
	...
	for token, constructor := range FnoExchanges {
		buildExchange(token, constructor, bot.FnoBtcExchanges)
	}

	if len(bot.FnoBtcExchanges) == 0 {
		return nil, fmt.Errorf("No FNO-BTC exchanges were intitialized")
	}
...
func (bot *ExchangeBot) Start(ctx context.Context, wg *sync.WaitGroup) {
	...
	if config.MasterBot != "" {
		...
		if err != nil {
			...
			// Start a loop to listen for updates from the fnorates server.
			go func() {
				for {
					...
					if err != nil {
						...
						log.Errorf("FNORates error. Attempting to reconnect in 10 seconds: %v", err)
						...
						for {
							...
							if err == nil {
								...
								log.Errorf("Failed to reconnect to FNORates. Attempting to reconnect in %s: %v", delayString, err)
							}
						}
						log.Infof("FNORates connection re-established.")
						reconnectionAttempt = 0
						continue
					}
					// Send the update through the Exchange so that appropriate attributes
					// are set.
					if IsFnoExchange(update.Token) {
...
// Attempt FNORates connection after delay.
func (bot *ExchangeBot) connectMasterBot(ctx context.Context, delay time.Duration) (fnorates.FNORates_SubscribeExchangesClient, error) {
	if bot.masterConnection != nil {
		bot.masterConnection.Close()
	}
	if delay > 0 {
	...
	grpcClient := fnorates.NewFNORatesClient(conn)
	stream, err := grpcClient.SubscribeExchanges(ctx, &fnorates.ExchangeSubscription{
...
func (bot *ExchangeBot) ConvertedState(code string) (*ExchangeBotState, error) {
	...
	fnoPrice, volume := bot.processState(bot.currentState.FnoBtc, true)
	btcPrice, _ := bot.processState(fiatIndices, false)
	if fnoPrice == 0 || btcPrice == 0 {
		bot.failed = true
	...
	state := ExchangeBotState{
		...
		Price:       fnoPrice * btcPrice,
		FnoBtc:      bot.currentState.FnoBtc,
...
// updateExchange processes an update from a Fonero-BTC Exchange.
func (bot *ExchangeBot) updateExchange(update *ExchangeUpdate) error {
	...
	bot.currentState.FnoBtc[update.Token] = update.State
...
func (bot *ExchangeBot) updateState() error {
	...
	if fnoPrice == 0 || btcPrice == 0 {
		...
		bot.failed = false
		bot.currentState.Price = fnoPrice * btcPrice
...
// sources or no Fonero Exchanges are up-to-date. Individual exchanges can
// be outdated/failed without IsFailed being false, as long as there is at least
// one Bitcoin index and one Fonero exchange.
...
// Conversion is a representation of some amount of FNO in another index.
...
func (bot *ExchangeBot) Conversion(fnoVal float64) *Conversion {
	if bot == nil {
		return nil
	}
	xcState := bot.State()
...
func (bot *ExchangeBot) QuickSticks(token string, rawBin string) ([]byte, error) {
	...
	bot.mtx.Lock()
	defer bot.mtx.Unlock()
	state, found := bot.currentState.FnoBtc[token]
	if !found {
		return nil, fmt.Errorf("Failed to find FNO exchange state for %s", token)
	}
...
func (bot *ExchangeBot) aggOrderbook() *aggregateOrderbook {
	...
	tokens := []string{}
	for token, xcState := range state.FnoBtc {
		if !xcState.HasDepth() {
			continue
		}
...
func (bot *ExchangeBot) QuickDepth(token string) (chart []byte, err error) {
	...
	if token == aggregatedOrderbookKey {
		...
		bot.mtx.Lock()
		defer bot.mtx.Unlock()
		xcState, found := bot.currentState.FnoBtc[token]
		if !found {
			return nil, fmt.Errorf("Failed to find FNO exchange state for %s", token)
		}
