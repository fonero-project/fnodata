...
type RateClient interface {
	SendExchangeUpdate(*fnorates.ExchangeRateUpdate) error
	Stream() GRPCStream
}
...
type GRPCStream interface {
	Send(*fnorates.ExchangeRateUpdate) error
...
// SubscribeExchanges is a gRPC method defined in fnorates/fnorates.proto. It
// satisfies the FNORatesServer interface. The gRPC server will call this method
// when a client subscription is received.
func (server *RateServer) SubscribeExchanges(hello *fnorates.ExchangeSubscription, stream fnorates.FNORates_SubscribeExchangesServer) (err error) {
...
func (server *RateServer) ReallySubscribeExchanges(hello *fnorates.ExchangeSubscription, stream GRPCStream) (err error) {
	...
	// Send Fonero exchanges.
	err = sendStateList(client, state.FnoBtc)
	if err != nil {
		return err
	}
	// Send Bitcoin-fiat indices.
	for token := range state.FiatIndices {
		err = client.SendExchangeUpdate(&fnorates.ExchangeRateUpdate{
			Token:   token,
...
func (server *RateServer) addClient(stream GRPCStream, hello *fnorates.ExchangeSubscription) (RateClient, StreamID) {
...
func makeExchangeRateUpdate(update *exchanges.ExchangeUpdate) *fnorates.ExchangeRateUpdate {
	state := update.State
	protoUpdate := &fnorates.ExchangeRateUpdate{
		Token:      update.Token,
		Price:      state.Price,
	...
	if state.Candlesticks != nil {
		protoUpdate.Candlesticks = make([]*fnorates.ExchangeRateUpdate_Candlesticks, 0, len(state.Candlesticks))
		for bin, sticks := range state.Candlesticks {
			candlesticks := &fnorates.ExchangeRateUpdate_Candlesticks{
				Bin:    string(bin),
				Sticks: make([]*fnorates.ExchangeRateUpdate_Candlestick, 0, len(sticks)),
			}
			for _, stick := range sticks {
				candlesticks.Sticks = append(candlesticks.Sticks, &fnorates.ExchangeRateUpdate_Candlestick{
					High:   stick.High,
					Low:    stick.Low,
					Open:   stick.Open,
					Close:  stick.Close,
	...
	if state.Depth != nil {
		depth := &fnorates.ExchangeRateUpdate_DepthData{
			Time: state.Depth.Time,
			Bids: make([]*fnorates.ExchangeRateUpdate_DepthPoint, 0, len(state.Depth.Bids)),
			Asks: make([]*fnorates.ExchangeRateUpdate_DepthPoint, 0, len(state.Depth.Asks)),
		}
		for _, pt := range state.Depth.Bids {
			depth.Bids = append(depth.Bids, &fnorates.ExchangeRateUpdate_DepthPoint{
				Quantity: pt.Quantity,
				Price:    pt.Price,
			})
		}
		for _, pt := range state.Depth.Asks {
			depth.Asks = append(depth.Asks, &fnorates.ExchangeRateUpdate_DepthPoint{
				Quantity: pt.Quantity,
				Price:    pt.Price,
			})
		}
...
func (client *rateClient) SendExchangeUpdate(update *fnorates.ExchangeRateUpdate) (err error) {
