...
type VoteDataSource interface {
	GetStakeVersionInfo(int32) (*fnojson.GetStakeVersionInfoResult, error)
	GetVoteInfo(uint32) (*fnojson.GetVoteInfoResult, error)
	GetStakeVersions(string, int32) (*fnojson.GetStakeVersionsResult, error)
}

// fnod does not supply vote counts for completed votes, so the tracker will
...
type VoteTracker struct {
	...
	blockVersion   int32
	stakeVersion   uint32
	stakeInfo      *fnojson.GetStakeVersionInfoResult
	voteInfo       *fnojson.GetVoteInfoResult
	summary        *VoteSummary
	ringIndex      int
	ringHeight     int64
	blockRing      []int32
...
// Since version could technically be updated without turning off fnodata,
...
func (tracker *VoteTracker) refreshRCI() (*fnojson.GetVoteInfoResult, error) {
	oldVersion := tracker.Version()
	v := oldVersion
	var err error
	var voteInfo, vinfo *fnojson.GetVoteInfoResult
...
func rciBlocks(voteInfo *fnojson.GetVoteInfoResult) int64 {
...
func (tracker *VoteTracker) fetchBlocks(voteInfo *fnojson.GetVoteInfoResult) ([]int32, uint32, error) {
	blocksToRequest := 1
	...
	var block fnojson.StakeVersions
...
func (tracker *VoteTracker) refreshSVIs(voteInfo *fnojson.GetVoteInfoResult) (*fnojson.GetStakeVersionInfoResult, error) {
...
// Once all resources have been retrieved from fnod, update VoteTracker fields.
func (tracker *VoteTracker) update(voteInfo *fnojson.GetVoteInfoResult, blocks []int32,
	stakeInfo *fnojson.GetStakeVersionInfoResult, stakeVersion uint32) {
