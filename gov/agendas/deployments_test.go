...
func (*testClient) GetVoteInfo(version uint32) (*fnojson.GetVoteInfoResult, error) {
	if version != 5 {
		return &fnojson.GetVoteInfoResult{}, fmt.Errorf(voteVersionErrMsg, version)
	}
	resp := &fnojson.GetVoteInfoResult{
		...
		Agendas: []fnojson.Agenda{
			{
				ID:             "TestAgenda0001",
				Description:    "This agenda just shows fnojson.GetVoteInfoResult payload format",
				Mask:           6,
