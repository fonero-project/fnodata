...
// AgendaTagged has the same fields as fnojson.Agenda plus the VoteVersion
...
type DeploymentSource interface {
	GetVoteInfo(version uint32) (*fnojson.GetVoteInfoResult, error)
}
...
// fnojson.GetVoteInfoResult and chaincfg.ConsensusDeployment hold almost similar
// data contents but chaincfg.Vote does not contain the important vote status
// field that is found in fnojson.Agenda.
