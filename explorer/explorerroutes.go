...
func (exp *explorerUI) Home(w http.ResponseWriter, r *http.Request) {
	...
	if xcBot != nil {
		conversions = &homeConversions{
			...
			CoinSupply:      xcBot.Conversion(fnoutil.Amount(homeInfo.CoinSupply).ToCoin()),
			PowSplit:        xcBot.Conversion(fnoutil.Amount(homeInfo.NBlockSubsidy.PoW).ToCoin()),
			TreasurySplit:   xcBot.Conversion(fnoutil.Amount(homeInfo.NBlockSubsidy.Dev).ToCoin()),
			TreasuryBalance: xcBot.Conversion(fnoutil.Amount(homeInfo.DevFund).ToCoin()),
		}
	}
...
func (exp *explorerUI) TxPage(w http.ResponseWriter, r *http.Request) {
	...
	if tx.Type == "Coinbase" {
		...
		if data == nil {
			...
			feeAmt, _ := fnoutil.NewAmount(data.MiningFee)
...
func (exp *explorerUI) AddressPage(w http.ResponseWriter, r *http.Request) {
	...
	conversion := exp.xcBot.Conversion(fnoutil.Amount(addrData.Balance.TotalUnspent).ToCoin())
...
func (exp *explorerUI) commonData(r *http.Request) *CommonPageData {
	...
	if err != nil && err != http.ErrNoCookie {
		log.Errorf("Cookie fnodataDarkBG retrieval error: %v", err)
	}
