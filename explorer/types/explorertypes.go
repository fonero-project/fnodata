...
func (mpi *MempoolInfo) Trim() *TrimmedMempoolInfo {
	...
	getTotalFee := func(txs []*TrimmedTxInfo) fnoutil.Amount {
		var sum fnoutil.Amount
		for _, tx := range txs {
			sum += tx.TxBasic.Fee
		}
		return sum
...
func TrimMempoolTx(txs []MempoolTx) (trimmedTxs []*TrimmedTxInfo) {
	for _, tx := range txs {
		fee, _ := fnoutil.NewAmount(tx.Fees) // non-nil error returns 0 fee
		var feeRate fnoutil.Amount
		if tx.Size > 0 {
			feeRate = fee / fnoutil.Amount(int64(tx.Size))
		}
		txBasic := &TxBasic{
			TxID:          tx.TxID,
