// Copyright (c) 2017-2019, The fnodata developers
...
type BlockGetter interface {
	...
	BestBlock() (*fnoutil.Block, error)
	Block(chainhash.Hash) (*fnoutil.Block, error)
...
// directly from fnod via RPC and subsequently update the internal block cache
// with the retrieved block.
type MasterBlockGetter interface {
	BlockGetter
	UpdateToBestBlock() (*fnoutil.Block, error)
	UpdateToNextBlock() (*fnoutil.Block, error)
	UpdateToBlock(height int64) (*fnoutil.Block, error)
}
...
type BlockGate struct {
	...
	blockWithHash map[chainhash.Hash]*fnoutil.Block
...
func NewBlockGate(client BlockFetcher, capacity int) *BlockGate {
	return &BlockGate{
		...
		blockWithHash: make(map[chainhash.Hash]*fnoutil.Block),
...
// NodeHeight gets the chain height from fnod.
...
func (g *BlockGate) BestBlock() (*fnoutil.Block, error) {
	g.mtx.RLock()
	defer g.mtx.RUnlock()
	var err error
	hash, ok := g.hashAtHeight[g.height]
...
func (g *BlockGate) CachedBlock(hash chainhash.Hash) (*fnoutil.Block, error) {
	g.mtx.RLock()
	defer g.mtx.RUnlock()
	block, ok := g.blockWithHash[hash]
	if !ok {
...
// the event of a cache miss, the block is retrieved from fnod via RPC.
func (g *BlockGate) Block(hash chainhash.Hash) (*fnoutil.Block, error) {
	...
	// Cache miss. Retrieve from fnod RPC.
...
func (g *BlockGate) UpdateToBestBlock() (*fnoutil.Block, error) {
	_, height, err := g.client.GetBestBlock()
	if err != nil {
...
// updates the cache.
func (g *BlockGate) UpdateToNextBlock() (*fnoutil.Block, error) {
	g.mtx.Lock()
	height := g.height + 1
	g.mtx.Unlock()
	return g.UpdateToBlock(height)
...
// fnod, stores it in cache, and signals any waiters. This is the thread-safe
// version of updateToBlock.
func (g *BlockGate) UpdateToBlock(height int64) (*fnoutil.Block, error) {
	g.mtx.Lock()
	defer g.mtx.Unlock()
	return g.updateToBlock(height)
}
...
// updateToBlock gets the block at the specified height on the main chain from
// fnod. It is not thread-safe. It wrapped by UpdateToBlock for thread-safety,
