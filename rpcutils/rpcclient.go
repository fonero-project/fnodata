...
func GetBlockHeaderVerbose(client BlockFetcher, idx int64) *fnojson.GetBlockHeaderVerboseResult {
...
func GetBlockHeaderVerboseByString(client BlockFetcher, hash string) *fnojson.GetBlockHeaderVerboseResult {
...
func GetBlock(ind int64, client BlockFetcher) (*fnoutil.Block, *chainhash.Hash, error) {
...
func GetBlockByHash(blockhash *chainhash.Hash, client BlockFetcher) (*fnoutil.Block, error) {
	msgBlock, err := client.GetBlock(blockhash)
	if err != nil {
...
func GetTransactionVerboseByID(client *rpcclient.Client, txhash *chainhash.Hash) (*fnojson.TxRawResult, error) {
...
// orphaned DB tip whose corresponding block is not known to fnod will not cause
...
func OrphanedTipLength(ctx context.Context, client BlockHashGetter,
	...
	for {
		...
		fnodHash, err = client.GetBlockHash(commonHeight)
		if err != nil {
			return -1, fmt.Errorf("Unable to retrive fnod block at height %d: %v", commonHeight, err)
		}
		if fnodHash.String() == dbHash {
			break
		}
		...
		// Reorgs are soft-limited to depth 6 by fnod. More than six blocks without
...
func UnconfirmedTxnsForAddress(client *rpcclient.Client, address string, params *chaincfg.Params) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := client.GetRawMempoolVerbose(fnojson.GRMAll)
	if err != nil {
