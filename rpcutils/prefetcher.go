...
type BlockFetcher interface {
	...
	GetBlockHeaderVerbose(hash *chainhash.Hash) (*fnojson.GetBlockHeaderVerboseResult, error)
}
...
type blockData struct {
	...
	headerResult *fnojson.GetBlockHeaderVerboseResult
}
...
func (p *BlockPrefetchClient) GetBlockData(hash *chainhash.Hash) (*wire.MsgBlock, *fnojson.GetBlockHeaderVerboseResult, error) {
	p.Lock()
...
// GetBlockHeaderVerbose retrieves the fnojson.GetBlockHeaderVerboseResult for
// the block with the specified hash. See GetBlockData for details on how this
// interacts with the prefetcher
func (p *BlockPrefetchClient) GetBlockHeaderVerbose(hash *chainhash.Hash) (*fnojson.GetBlockHeaderVerboseResult, error) {
...
// becomes "current".
func (p *BlockPrefetchClient) storeNext(msgBlock *wire.MsgBlock, headerResult *fnojson.GetBlockHeaderVerboseResult) {
	p.current = p.next
	p.next = &blockData{
...
func (p *BlockPrefetchClient) retrieveBlockAndHeaderResult(hash *chainhash.Hash) (*wire.MsgBlock, *fnojson.GetBlockHeaderVerboseResult, uint32, error) {
