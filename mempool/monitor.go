...
type MempoolMonitor struct {
	...
	// Incoming data
	newTxIn <-chan *fnojson.TxRawResult
	// Outgoing message
...
func NewMempoolMonitor(ctx context.Context, collector *MempoolDataCollector,
	...
	newTxInChan <-chan *fnojson.TxRawResult, signalOuts []chan<- pstypes.HubMessage,
...
func (p *MempoolMonitor) TxHandler(client *rpcclient.Client) {
	...
	for {
		...
		case s, ok := <-p.newTxIn:
			...
			case "Vote":
				// Votes on the next block may be received just prior to fnodata
