...
func (p *mempoolMonitor) TxHandler(client *rpcclient.Client) {
	...
	for {
		...
		case s, ok := <-p.newTxHash:
			...
			if err != nil {
				log.Errorf("Failed to get transaction (do you have --txindex with fnod?) %v: %v",
					s.Hash.String(), err)
				continue
			}
			...
			//s.Tree() == fnoutil.TxTreeRegular
			// See fnod/blockchain/stake/staketx.go for information about
			...
			case stake.TxTypeSStx:
				...
				log.Tracef("Received ticket purchase %v, price %v",
					tx.Hash(), fnoutil.Amount(price).ToCoin())
...
type MempoolData struct {
	...
	NumVotes          uint32
	NewTickets        uint32
	Ticketfees        *fnojson.TicketFeeInfoResult
