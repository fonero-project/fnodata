...
func NewMempoolDataCollector(fnodChainSvr *rpcclient.Client, params *chaincfg.Params) *MempoolDataCollector {
...
func (t *MempoolDataCollector) mempoolTxns() ([]exptypes.MempoolTx, txhelpers.MempoolAddressStore, txhelpers.TxnsStore, error) {
	mempooltxs, err := t.fnodChainSvr.GetRawMempoolVerbose(fnojson.GRMAll)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GetRawMempoolVerbose failed: %v", err)
	}

	blockHash, _, err := t.fnodChainSvr.GetBestBlock()
	if err != nil {
	...
	for hashStr, tx := range mempooltxs {
		...
		rawtx, err := rpcutils.GetTransactionVerboseByID(t.fnodChainSvr, hash)
		if err != nil {
			log.Warn(err)
			continue
		}
		...
		txhelpers.TxPrevOutsByAddr(addrMap, txnsStore, msgTx, t.fnodChainSvr, t.activeChain)
...
func (t *MempoolDataCollector) Collect() (*StakeData, []exptypes.MempoolTx, txhelpers.MempoolAddressStore, txhelpers.TxnsStore, error) {
	...
	mempoolTickets, err := c.GetRawMempoolVerbose(fnojson.GRMTickets)
	if err != nil {
		return nil, nil, nil, nil, err
	}

	mempoolVotes, err := c.GetRawMempoolVerbose(fnojson.GRMVotes)
	if err != nil {
		return nil, nil, nil, nil, err
	}
...
func ParseTxns(txs []exptypes.MempoolTx, params *chaincfg.Params, lastBlock *BlockID) *exptypes.MempoolInfo {
	...
	var totalOut, regularTotal, ticketTotal, voteTotal, revTotal fnoutil.Amount
	var likelyMineable bool
	var likelyTotal fnoutil.Amount
	...
	for _, tx := range txs {
		likelyMineable = true
		out, _ := fnoutil.NewAmount(tx.TotalOut) // 0 for invalid amounts
		switch tx.Type {
		case "Ticket":
