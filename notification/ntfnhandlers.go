...
func RegisterNodeNtfnHandlers(fnodClient *rpcclient.Client) *ContextualError {
	...
	// fnodClient.NotifyNewTickets()

	if err = fnodClient.NotifyWinningTickets(); err != nil {
	...
	// if len(addresses) > 0 {
	// 	if err = fnodClient.LoadTxFilter(true, addresses, nil); err != nil {
...
// signalReorg takes the basic reorganization data from fnod, determines the two
...
func signalReorg(d ReorgData) {
	...
	// Send reorg data to fnosqlite's monitor.
	d.WG.Add(1)
	select {
	...
	d.WG.Add(1)
	select {
	case NtfnChans.ReorgChanFnopgDB <- fullData:
	default:
		d.WG.Done()
	}
	d.WG.Wait()
...
// MakeNodeNtfnHandlers defines the fnod notification handlers.
func MakeNodeNtfnHandlers() (*rpcclient.NotificationHandlers, *collectionQueue) {
	...
	return &rpcclient.NotificationHandlers{
		...
		// for the mempool monitors to avoid an extra call to fnod for
		// the tx details
		OnTxAcceptedVerbose: func(txDetails *fnojson.TxRawResult) {
