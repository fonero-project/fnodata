...
type RawTransactionGetter interface {
	GetRawTransaction(txHash *chainhash.Hash) (*fnoutil.Tx, error)
}
...
type BlockWatchedTx struct {
	BlockHeight   int64
	TxsForAddress map[string][]*fnoutil.Tx
}
...
// TxhashInSlice searches a slice of *fnoutil.Tx for a transaction with the hash
// txHash. If found, it returns the corresponding *Tx, otherwise nil.
func TxhashInSlice(txs []*fnoutil.Tx, txHash *chainhash.Hash) *fnoutil.Tx {
	if len(txs) < 1 {
		return nil
	}
...
func IncludesStakeTx(txHash *chainhash.Hash, block *fnoutil.Block) (int, int8) {
...
func IncludesTx(txHash *chainhash.Hash, block *fnoutil.Block) (int, int8) {
...
func BlockConsumesOutpointWithAddresses(block *fnoutil.Block, addrs map[string]TxAction,
	c RawTransactionGetter, params *chaincfg.Params) map[string][]*fnoutil.Tx {
	addrMap := make(map[string][]*fnoutil.Tx)

	checkForOutpointAddr := func(blockTxs []*fnoutil.Tx) {
					...
					for _, txAddr := range txAddrs {
						...
						if _, ok := addrs[addrstr]; ok {
							if addrMap[addrstr] == nil {
								addrMap[addrstr] = make([]*fnoutil.Tx, 0)
							}
...
// specified addresses, and creates a map of addresses to a slice of fnoutil.Tx
// involving the address.
func BlockReceivesToAddresses(block *fnoutil.Block, addrs map[string]TxAction,
	params *chaincfg.Params) map[string][]*fnoutil.Tx {
	addrMap := make(map[string][]*fnoutil.Tx)

	checkForAddrOut := func(blockTxs []*fnoutil.Tx) {
		for _, tx := range blockTxs {
			// Check the addresses associated with the PkScript of each TxOut
				...
				for _, txAddr := range txOutAddrs {
					...
					if _, ok := addrs[addrstr]; ok {
						if _, gotSlice := addrMap[addrstr]; !gotSlice {
							addrMap[addrstr] = make([]*fnoutil.Tx, 0) // nil
						}
...
func OutPointAddresses(outPoint *wire.OutPoint, c RawTransactionGetter,
	params *chaincfg.Params) ([]string, fnoutil.Amount, error) {
	...
	value := fnoutil.Amount(txOut.Value)
...
func MedianAmount(s []fnoutil.Amount) fnoutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(fnoutil.AmountSorter(s))
...
// MedianCoin gets the median FNO from a slice of float64s
...
func SSTXInBlock(block *fnoutil.Block) []*fnoutil.Tx {
...
func SSGenVoteBlockValid(msgTx *wire.MsgTx) (BlockValidation, uint16, error) {
	...
	blockValid := BlockValidation{
		...
		Validity: fnoutil.IsFlagSet16(ssGenVoteBits, fnoutil.BlockValid),
	}
	return blockValid, ssGenVoteBits, nil
}

// VoteBitsInBlock returns a list of vote bits for the votes in a block
func VoteBitsInBlock(block *fnoutil.Block) []stake.VoteVersionTuple {
...
// specified block.
func FeeInfoBlock(block *fnoutil.Block) *fnojson.FeeInfoBlock {
	feeInfo := new(fnojson.FeeInfoBlock)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := fnoutil.Amount(amtIn - amtOut).ToCoin()
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func FeeRateInfoBlock(block *fnoutil.Block) *fnojson.FeeInfoBlock {
	feeInfo := new(fnojson.FeeInfoBlock)
	_, sstxMsgTxns := TicketsInBlock(block)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := fnoutil.Amount(1000*(amtIn-amtOut)).ToCoin() / float64(msgTx.SerializeSize())
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func TxFee(msgTx *wire.MsgTx) fnoutil.Amount {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return fnoutil.Amount(amtIn - amtOut)
}

// TxFeeRate computes and returns the fee rate in FNO/KB for a given tx
func TxFeeRate(msgTx *wire.MsgTx) (fnoutil.Amount, fnoutil.Amount) {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return fnoutil.Amount(amtIn - amtOut), fnoutil.Amount(1000 * (amtIn - amtOut) / int64(msgTx.SerializeSize()))
}

// TotalOutFromMsgTx computes the total value out of a MsgTx
func TotalOutFromMsgTx(msgTx *wire.MsgTx) fnoutil.Amount {
	var amtOut int64
	for _, v := range msgTx.TxOut {
		amtOut += v.Value
	}
	return fnoutil.Amount(amtOut)
}

// TotalVout computes the total value of a slice of fnojson.Vout
func TotalVout(vouts []fnojson.Vout) fnoutil.Amount {
	var total fnoutil.Amount
	for _, v := range vouts {
		a, err := fnoutil.NewAmount(v.Value)
		if err != nil {
			continue
		}
		total += a
...
func IsZeroHashP2PHKAddress(checkAddressString string, params *chaincfg.Params) bool {
	...
	address, err := fnoutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return false
	}
...
// network.
func ValidateNetworkAddress(address fnoutil.Address, p *chaincfg.Params) bool {
...
// string. Initially, decoding as a Fonero address is attempted. If it fails to
...
// AddressTypeBitcoin. If the address decoded successfully as a Fonero address,
...
// address type.
func AddressValidation(address string, params *chaincfg.Params) (fnoutil.Address, AddressType, AddressError) {
	...
	// Detect when an address belonging to a different Fonero network.
	if !ValidateNetworkAddress(addr, params) {
		return addr, AddressTypeUnknown, AddressErrorWrongNet
	}

	// Determine address type for this valid Fonero address. Ignore the error
