...
import (
	...
	_ "github.com/fonero-project/fnod/database/ffldb" // init the ffldb driver
...
func BuildStakeTree(blocks map[int64]*fnoutil.Block, netParams *chaincfg.Params,
...
func TicketsInBlock(bl *fnoutil.Block) ([]chainhash.Hash, []*wire.MsgTx) {
...
func TicketTxnsInBlock(bl *fnoutil.Block) ([]chainhash.Hash, []*fnoutil.Tx) {
	tickets := make([]chainhash.Hash, 0)
	ticketTxns := make([]*fnoutil.Tx, 0)
	for _, stx := range bl.STransactions() {
		if stake.DetermineTxType(stx.MsgTx()) == stake.TxTypeSStx {
			h := stx.Hash()
			tickets = append(tickets, *h)
...
func TicketsSpentInBlock(bl *fnoutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
...
func VotesInBlock(bl *fnoutil.Block) []chainhash.Hash {
...
func RevokedTicketsInBlock(bl *fnoutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
