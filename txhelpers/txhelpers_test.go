...
type TxGetter struct {
	txLookup map[chainhash.Hash]*fnoutil.Tx
}

func (t TxGetter) GetRawTransaction(txHash *chainhash.Hash) (*fnoutil.Tx, error) {
...
func LoadTestBlockAndSSTX(t *testing.T) (*fnoutil.Block, []*fnoutil.Tx) {
	// Load block data
	...
	block, err := fnoutil.NewBlockFromReader(blockTestFile)
	if err != nil {
	...
	allTxRead := make([]*fnoutil.Tx, numSSTX)
	for i := range allTxRead {
		...
		allTxRead[i], err = fnoutil.NewTxFromReader(txFile)
		if err != nil {
			t.Fatal(err)
		}
...
func TestFeeRateInfoBlock(t *testing.T) {
	...
	fibExpected := fnojson.FeeInfoBlock{
		Height: 138883,
		Number: 20,
...
func TxToWriter(tx *fnoutil.Tx, w io.Writer) error {
	msgTx := tx.MsgTx()
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool) (*rpcclient.Client, semver.Semver, error) {
	...
	fnodClient, err := rpcclient.New(connCfgDaemon, nil)
	if err != nil {
